...
describe('creates a link attribute', () => {
    it('extracts a full url from a string', () => {
        ...
        assert.equal(new Link(fullUrl, 'http://whatever.com').value, 'http://whatever.com/whatever');
    });

    it('extracts a relative url from a string', () => {
        const relativeUrl = `/whatever/another`;
        assert.equal(new Link(relativeUrl, 'http://whatever.com/').value, 'http://whatever.com/whatever/another');
    });

    it('returns null if the link is empty', () => {
        assert.equal(new Link('', 'whatever').value, null);
    });
});
